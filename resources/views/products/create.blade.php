...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">{{ __('prodects.create_product') }}</div>
                    <div class="card-body">
                        ...
                        @endif
                            ...
                            <form action="{{ route('products.store') }}" method="post">
                                ...
                                @foreach (config('locales.languages') as $key => $value)
                                    ...
                                    <div class="tab-pane fade {{ $loop->index == 0 ? 'active show': '' }}" id="{{ $key }}"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="title[{{ $key }}]" value="{{ old('title'.$key) }}" class="form-control" placeholder="Enter Title Product" aria-describedby="helpId">
                                            @error('title') <snap class="text-danger">{{ $message }}</snap>  @enderror
                                        </div>
                                        <div class="form-group">
                                            <label for="dec">{{ __('prodects.dec') }} ({{ $key }})</label>
                                            <textarea class="form-control" name="dec[{{ $key }}]" value="{{ old('dec'.$key) }}" rows="3"></textarea>
                                            @error('dec') <snap class="text-danger">{{ $message }}</snap>  @enderror
                                        </div>
                                        <div class="form-group">
                                            <label for="price">{{ __('prodects.price') }} ({{ $key }})</label>
                                            <input type="text" class="form-control" name="price[{{ $key }}]" value="{{ old('price'.$key) }}" aria-describedby="helpId" placeholder="0.0">
                                ...
                                <div class="form-group">
                                    <button type="submit"  name="submit" class="btn btn-primary"> {{ __('prodects.create_product') }}</button>
                                </div>
                            </form>
                    </div>
